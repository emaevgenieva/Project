...
typedef struct InvoicesData {
    char cType[TYPE_SIZE];
    char cCustomersExternalReference[CUSTOMERS_INT_SIZE];
...
int validateFileName(FILE* fpMsg, FILE* fpLog, char* fileName);
int countRowsInFile(FILE* fp);

int validationRow(FILE* fpMsg, FILE* fpLog, FILE* fp, char cRowForType[], char cRow[]);
void fillArray(FILE* fp, char cRow[]);
int checkForNull01(char cRow[]);

int validateRow01(FILE* fpMsg, FILE* fpLog, FILE* fp, int result);
...
int validateRow02(FILE* fpMsg, FILE* fpLog, FILE* fp, int result);
int checkInvoiceDate(const char* str);
int checkInvoiceSum(const char* str);
int checkInvoiceCurrency(const char* str);

int validateRow03(FILE* fpMsg, FILE* fpLog, FILE* fp, int result);
...
void insertCustomersInDb(FILE* fpMsg, FILE* fpLog, CustomersData* cd, char cRow[]);
void insertInvoicesInDb(FILE* fpMsg, FILE* fpLog, InvoicesData* id, char cRow[]);
void insertPaymentsInDb(FILE* fpMsg, FILE* fpLog, PaymentsData* pd, char cRow[]);
